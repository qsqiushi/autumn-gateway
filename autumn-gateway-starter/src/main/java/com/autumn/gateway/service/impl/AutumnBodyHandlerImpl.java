...
public class AutumnBodyHandlerImpl implements BodyHandler {

  private static final Logger LOG =
      LoggerFactory.getLogger(io.vertx.ext.web.handler.impl.BodyHandlerImpl.class);
  ...
  public AutumnBodyHandlerImpl() {
    this(true, DEFAULT_UPLOADS_DIRECTORY);
  }

  public AutumnBodyHandlerImpl(boolean handleFileUploads) {
    this(handleFileUploads, DEFAULT_UPLOADS_DIRECTORY);
  }

  public AutumnBodyHandlerImpl(String uploadDirectory) {
    this(true, uploadDirectory);
  }

  private AutumnBodyHandlerImpl(boolean handleFileUploads, String uploadDirectory) {
  ...
  public void handle(RoutingContext context) {
    ...
    if (request.version() == HttpVersion.HTTP_2
        && APPLICATION_GRPC.equals(request.getHeader(HttpHeaders.CONTENT_TYPE))) {
      return;
    }
    ...
    if (!((RoutingContextInternal) context).seenHandler(RoutingContextInternal.BODY_HANDLER)) {
      ...
      AutumnBodyHandlerImpl.BHandler handler =
          new AutumnBodyHandlerImpl.BHandler(context, contentLength);
  ...
  private class BHandler implements Handler<Buffer> {
    ...
    public BHandler(RoutingContext context, long contentLength) {
      ...
      if (contentType == null) {
        ...
        isUrlEncoded =
            lowerCaseContentType.startsWith(
                ...
                upload -> {
                  ...
                  if (handleFileUploads) {
                    ...
                    String uploadedFileName =
                        new File(uploadsDir, UUID.randomUUID().toString()).getPath();
